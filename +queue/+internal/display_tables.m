function [] = display_tables(data, disp_options)
%DISPLAY_TABLE displays table of data according to the given display options
%
% Copyright (c) 2017, Christian Rohlfing, Julian Becker, Jens Schneider, Patrick Wilken, Tobias Kabzinski

    % Workaround to allow 1D-tables: get plot dimensions that actually exists and create empty tick labels
    x_y_dims_existing = [];
    if isfield(disp_options.plot_x, 'dim');
        x_y_dims_existing(end + 1) = disp_options.plot_x.dim;
    else
        disp_options.plot_x.tick_labels = '';
    end
    
    if isfield(disp_options.plot_y, 'dim');
        x_y_dims_existing(end + 1) = disp_options.plot_y.dim;
    else
        disp_options.plot_y.tick_labels = '';
    end
    
    % Calculate number of elements in table
    numel_table = disp_options.plot_x.numel * disp_options.plot_y.numel;
    
    % Calculate number of tables
    number_of_tables = numel(data) / numel_table;

    % Ask if number of tables is too high
    if number_of_tables > 20
        reply = input(sprintf('Do you really want to display %d tables? [Y/n]\n', number_of_tables), 's');
        if strcmpi(reply, 'n')
            return
        end
    end

    % Ask if tables are too big
    if numel_table > 400
        reply = input(spritf('Do you really want to display tables with %d entries? [Y/n]\n', numel_table), 's');
        if strcmpi(reply, 'n')
            return
        end
    end
    ...
    % Store size data
    data_size = size(data);
    ndim_data = find(data_size ~= 1, 1, 'last');
    other_data_size = data_size(setdiff(1:ndim_data, x_y_dims_existing));
    
    % Iterate over tables to be displayed
    for table_id = 1:number_of_tables
        
        % Print heading
        fprintf('-TABLE %d-\n\n', table_id)

        % Get para value ids for current table
        other_para_value_ids = cell(ndim_data - length(x_y_dims_existing), 1);
        if ~isempty(other_data_size)
        	[other_para_value_ids{:}] = ind2sub(other_data_size, table_id);
        end     

        % Print values of parameter not displayed in table ('other_parameters')
        longest_para_label_length = max(cellfun(@length, disp_options.other_parameters.label));
        if ~isempty(longest_para_label_length)
            for para_id = 1:length(other_para_value_ids)
                fprintf(['  %-', int2str(longest_para_label_length + 1), 's %s\n'],...
                    [disp_options.other_parameters.label{para_id},':'], disp_options.other_parameters.tick_labels{para_id}{other_para_value_ids{para_id}})
            end
            fprintf('\n');
        end

        % Get data indices for current table
        data_indices = repmat({':'}, [1, ndim_data]);
        data_indices(setdiff(1:ndim_data, x_y_dims_existing)) = other_para_value_ids;

        % Apply data indices and squeeze data
        data_to_plot = data(data_indices{:});
        data_to_plot = squeeze(data_to_plot);

        % Get right order of plot axes
        if length(x_y_dims_existing) == 2
            [~, order] = sort(x_y_dims_existing);
            data_to_plot = permute(data_to_plot, fliplr(order));
        else
            % In case of 1D table decide between row and column vector
        ...
        % Display table
        displaytable(data_to_plot, disp_options.plot_x.tick_labels, '', '', disp_options.plot_y.tick_labels, 1, '|');
        fprintf('\n\n');
    end
...
%         fmt = arrayfun(@(x) ['%',num2str(wid(x)),'s |'], 1:nColD, 'UniformOutput', false);
        ...
        fprintf(fid, [repmat('_', 1, sum(wid) + rhwid + numel(wid)), '\n']);
