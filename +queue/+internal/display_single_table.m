function [] = display_single_table(results)
...
% Copyright (c) 2017, Christian Rohlfing, Julian Becker, Jens Schneider, Patrick Wilken, Tobias Kabzinski
    ...
    measure_names = queue.internal.get_para_names(results.data);
    first_measure_data = queue.internal.getfield_by_str(results.data, measure_names{1});
    numel_data = numel(first_measure_data);
    
    % Ask if number of tables is too high
    if numel_data > 200
        reply = input(sprintf('Do you really want to display a table with %d rows? [Y/n]\n', numel_data), 's');
        if strcmpi(reply, 'n')
            return
        end
    end

    % Get number of parameter values and initialize parameter value ids
    num_of_values = cellfun(@numel, results.para_values);
    para_value_ids = cell(length(num_of_values), 1);
    
    % Initialize table strings
    table_strings = cell(numel_data, length(results.para_names) + length(measure_names));
    
    % Create extra line between parameters and measures
    results.para_names{end} = [results.para_names{end}, ' |'];
    
    % Initialize required column widths with length of column headings (parameter names)
    widths = cellfun(@length, [results.para_names', measure_names']);
    
    % Iterate over rows to be displayed
    for combo_id = 1:numel_data
        
        if ~isempty(results.para_values)
            ...
            for para_id = 1:length(results.para_names)
                % Get string for current parameter value
                value_str = queue.internal.value2str(results.para_values{para_id}{para_value_ids(para_id)});
                
                % Create extra line between parameters and measures
                if para_id == length(results.para_names)
                    value_str = [value_str, ' |']; %#ok<AGROW>
                end
                
                % Assign parameter value to table data
                table_strings{combo_id, para_id} = value_str;
                
                % Update required column width for this parameter
                if widths(para_id) < length(value_str)
                    widths(para_id) = length(value_str);
                end
            end            
        end
        
        % Set measure values
        para_value_ids = num2cell(para_value_ids);
        for measure_id = 1:length(measure_names)
            % Get data for current measure
            measure_data = queue.internal.getfield_by_str(results.data, measure_names{measure_id});
            
            % Create indices
            ndim_data = find(size(measure_data) ~= 1, 1, 'last');
            data_indices = repmat({':'}, [1, ndim_data]);
            data_indices(1:length(para_value_ids)) = para_value_ids;
            
            % Assign measure value to table data
            value_str = queue.internal.value2str(squeeze(measure_data(data_indices{:})));
            table_strings{combo_id, length(results.para_names) + measure_id} = value_str;
            
            % Update required column width for this parameter
            if widths(length(results.para_names) + measure_id) < length(value_str)
                widths(length(results.para_names) + measure_id) = length(value_str);
            end
        end
    end
    ...
    % Display table
    fprintf('\n');
    displaytable(table_strings, [results.para_names', measure_names'], widths + 1, '', '', 1, '|');
...
%         fmt = arrayfun(@(x) ['%',num2str(wid(x)),'s |'], 1:nColD, 'UniformOutput', false);
        ...
        fprintf(fid, [repmat(' ', 1, rhwid), repmat('_', 1, sum(wid) + numel(wid)), '\n']);
